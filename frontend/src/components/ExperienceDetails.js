...
const ExperienceDetails = () => {
    const { id } = useParams();
    const { userId: loggedInUserId } = useContext(AuthContext); // Dohvatanje ID-a prijavljenog korisnika
    const [experience, setExperience] = useState(null);
    const [newComment, setNewComment] = useState('');
    const [currentImageIndex, setCurrentImageIndex] = useState(0);
    const [editingCommentId, setEditingCommentId] = useState(null); // State za praćenje koji komentar se edituje
    const [updatedComment, setUpdatedComment] = useState(''); // State za ažurirani tekst komentara
    ...
    const imageUrl = experience.images?.[currentImageIndex]
        ? `http://localhost:8080/uploads/${experience.images[currentImageIndex].split('\\').pop()}`
        : defaultPhoto;
        ...
        <div>
            ...
            <div className="experience-details-info">
                <div className="experience-detail">
                    <strong>Destinacija:</strong> {experience.destination?.locationName || ''}{experience.destination?.regionArea ? `, ${experience.destination.regionArea}` : ''}{experience.destination?.country?.countryName ? `, ${experience.destination.country.countryName}` : ''}
                </div>
                <div className="experience-detail">
    ...
    experience.comments.map((comment) => (
        <div key={comment.id} className="comment-item">
            ...
            {/* Editable div ako je komentar u fazi editovanja, inače prikazuje običan div */}
            {editingCommentId === comment.id ? (
                <div
                    className="comment-content editable"
                    contentEditable={true}
                    suppressContentEditableWarning={true}
                    onBlur={(e) => setUpdatedComment(e.target.textContent)} // Čuvanje izmenjenog teksta
                >
                    {comment.content}
                </div>
            ) : (
            ...
            {comment.appUser?.id === loggedInUserId && (
                <div className="comment-actions">
                    {/* Menjanje dugmeta "Izmeni" u "Sačuvaj" ako je u fazi izmene */}
                    {editingCommentId === comment.id ? (
                        <button
                        ...
                        <button
                            className="add-comment-button"
                            onClick={() => {
                                setEditingCommentId(comment.id); // Postavljanje ID-a komentara koji se edituje
                                setUpdatedComment(comment.content); // Postavljanje trenutnog teksta komentara u state
                            }}
                        >
                            Izmeni
                        </button>
